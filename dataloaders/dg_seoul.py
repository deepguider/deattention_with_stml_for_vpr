...
def get_whole_training_set(onlyDB=False):  # Provide all DB and Q image
    ...
    structFile = join(struct_dir, 'dg_seoul_train.mat')
    return WholeDatasetFromStruct(structFile,
                             input_transform=input_transform(),
                             onlyDB=onlyDB)

def get_whole_val_set():
    structFile = join(struct_dir, 'dg_seoul_val.mat')
    return WholeDatasetFromStruct(structFile,
                             input_transform=input_transform())

def get_whole_test_set(onlyQ=False):
    structFile = join(struct_dir, 'dg_seoul_test.mat')
    return WholeDatasetFromStruct(structFile,
                             input_transform=input_transform(),
                             onlyQ=onlyQ)
...
def Load_dbStruct(fname):
    ...
    dbImage = [f.item().strip() for f in matStruct[2].squeeze()]
    utmDb = matStruct[3]
    
    qImage = [f.item().strip() for f in matStruct[4].squeeze()]
    utmQ = matStruct[5]
