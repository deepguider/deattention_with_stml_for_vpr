...
def get_whole_training_set(onlyDB=False):  # Provide all DB and Q image
    ...
    structFile = join(struct_dir, 'dg_bucheon_train.mat')
    return WholeDatasetFromStruct(structFile,
                             input_transform=input_transform(),
                             onlyDB=onlyDB)

def get_whole_val_set():
    structFile = join(struct_dir, 'dg_bucheon_val.mat')
    return WholeDatasetFromStruct(structFile,
                             input_transform=input_transform())

def get_whole_test_set(onlyQ=False):
    structFile = join(struct_dir, 'dg_bucheon_test.mat')
    return WholeDatasetFromStruct(structFile,
                             input_transform=input_transform(),
                             onlyQ=onlyQ)
...
class QueryDatasetFromStruct(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.disp_en and (disp_stack_cnt == 0):  # for debugging
            ...
            plt.pause(0.1)
            if False:
                negative_tmp1 = Image.open(join(db_dir, self.dbStruct.dbImage[negSample[negNN_bakup[17]].astype(np.int32)]))
                negative_tmp2 = Image.open(join(db_dir, self.dbStruct.dbImage[negSample[negNN_bakup[18]].astype(np.int32)]))
