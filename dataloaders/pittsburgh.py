...
root_dir = '/home/ccsmm/DB_Repo/Pittsburgh250k/netvlad_v100_datasets'
...
db_dir = join(root_dir, '.')  # db locates at root_dir
queries_dir = join(root_dir, 'queries_real') # query locates at queries_real, data set from real camera such as smartphone
...
class WholeDatasetFromStruct(data.Dataset):  ## For evaluation
    def __init__(self, structFile, input_transform=None, onlyDB=False):
        super().__init__()

        self.input_transform = input_transform
        if ('naver' in structFile.split('/')[-1]) or  ('dg' in structFile.split('/')[-1]) :
    ...
    def __getitem__(self, index):
        ...
        if False:  # ori, use knn of sklearn
            with h5py.File(self.cache, mode='r') as h5:   ## use knn of sklearn
                ...
                #violatingNeg = dNeg < dPos + self.margin**0.5  # ori from git
                violatingNeg = dNeg < dPos + self.margin  # paper
            ...
            with h5py.File(self.cache, mode='r') as h5:  ## use knn of faiss
                ...
                # Try to find hard-negatives that are within margin, if there aren't any return none
                # ViolatingNeg means hard-negative image whose feature is more similar to query than postive image is.
                #violatingNeg = dNeg < dPos + self.margin**0.5  #ori of git
                violatingNeg = dNeg < dPos + self.margin  # paper
